<script lang="ts">
  ...
  import EditPlaylistDrawer from "$lib/components/content/drawer-contents/playlist-edit-drawer.svelte";
  ...
  function openPlaylistEditDrawer() {
    drawerState.open({
      component: EditPlaylistDrawer,
      props: { form, playlist },
      title: "Edit Playlist",
      options: {
        fullHeight: true,
  ...
  <div class="flex flex-col gap-4 m-4">
    ...
    <div class="flex flex-col @md:flex-row gap-6">
        ...
        <PlaylistEditDialog {form} {playlist} bind:open>
          <div class="flex justify-center">
            {#if playlist.processedImageUrl}
              <button
                type="button"
                class="outline-none flex justify-center items-center h-56 w-56 {isPlaylistOwner &&
            ...
            {:else}
              <button
                type="button"
                class="outline-none flex justify-center items-center min-h-32 min-w-32 h-56 w-56 {isPlaylistOwner &&
                  'cursor-pointer'}border-none bg-transparent p-0"
                onclick={openDialog}
              >
                <ListVideo size={128} />
        ...
        <div class="flex justify-center">
          {#if playlist.processedImageUrl}
            <button
              ...
              class="outline-none flex justify-center items-center h-56 w-56 {isPlaylistOwner &&
                'cursor-pointer'} border-none bg-transparent p-0"
              onclick={openPlaylistEditDrawer}
            >
              <img
                src={playlist.processedImageUrl}
                alt={`Image for playlist: ${playlist.name}`}
      ...
      <div class="flex flex-col relative flex-1 min-w-2xs">
        <button
          type="button"
          class="outline-none flex flex-col {isPlaylistOwner &&
            'cursor-pointer'} 
        ...
        >
          ...
          {#if playlist.description}
            <p
              class="w-full text-sm text-muted-foreground text-left break-words mb-2"
            >
