  ...
  onMount(() => {
    if (isBrowser()) {
      ...
      if (continueWatchingVideos.length > 0) {
        const continueWatchingUrls = extractImageUrls(
          continueWatchingVideos.slice(0, 5)
        );
      ...
      Object.entries(sourceVideos).forEach(([source, videos]) => {
        if (videos.length > 0) {
          const firstFewUrls = extractImageUrls(videos.slice(0, 4));
          preloadImages(firstFewUrls, {
            priority: source === 'YOUTUBE' ? 'high' : 'auto',
          });
        }
      });
    }
