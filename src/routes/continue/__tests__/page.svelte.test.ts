...
describe('continue/+page.svelte Component Logic', () => {
  ...
  describe('content header configuration', () => {
    it('should pass correct props to ContentHeader', () => {
      const currentPage = 2;
      const showFloatingBreadcrumbs = true;
      ...
      const headerProps = {
        ...
        userProfile: mockData.userProfile,
        videosCount: mockData.videosCount ?? 0,
        currentPage,
        showFloatingBreadcrumbs,
        breadcrumbs: [
          {
      ...
      expect(headerProps.title).toBe('Continue Watching');
      expect(headerProps.view).toBe('continueWatching');
      expect(headerProps.videosCount).toBe(25);
      expect(headerProps.breadcrumbs).toHaveLength(2);
      expect(headerProps.breadcrumbs[1].label).toBe('Page 2');
    });

    it('should handle null videosCount in header props', () => {
      const dataWithNullCount = {
        ...mockData,
        videosCount: null,
      };

      const videosCount = dataWithNullCount.videosCount ?? 0;
