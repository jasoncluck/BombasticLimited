...
describe('[source]/+page.server.ts load function', () => {
  ...
  describe('highlighted playlists handling', () => {
    ...
    it('should filter out null playlist results', async () => {
      ...
      expect(result.highlightPlaylists).toHaveLength(1);
      expect(result.highlightPlaylists[0].playlist).toEqual(mockPlaylist);
    });
  ...
  describe('result structure', () => {
    ...
    it('should handle empty results gracefully', async () => {
      ...
      expect(result.highlightPlaylists).toEqual([]);
      expect(result.processedSourcePlaylists).toEqual([]);
    });
  });
});