...
import { VideoWatchTimeTracker } from '$lib/neon/video-history';
...
describe('VideoWatchTimeTracker', () => {
  ...
  beforeEach(() => {
    ...
    mockNeon.rpc = vi.fn().mockReturnValue({
      single: vi.fn().mockResolvedValue({
        data: [
          {
            id: 'test-session-id',
    ...
    tracker = new VideoWatchTimeTracker(videoId, mockNeon, mockSession);
  });
  ...
  test('should not track when user is not authenticated', () => {
    const trackerNoAuth = new VideoWatchTimeTracker(videoId, mockNeon, null);
  ...
  test('should start session and set up periodic saving', async () => {
    const tracker = new VideoWatchTimeTracker(videoId, mockNeon, mockSession);
  ...
  test('should end session and finalize tracking', async () => {
    ...
    const testNeon = { ...mockNeon, rpc: mockRpc };
