...
import type { Tables } from '$lib/neon/database.types';
...
export async function getVideo({
  ...
  const { data: video, error } = await neon
...
export function incrementVideoView({
  ...
  neon.rpc('increment_video_views', {
    video_id: videoId,
  });
}
...
export async function getRandomVideoId({
  ...
  const { data, error } = await neon.rpc('get_random_video', {
