...
vi.mock('../playlists/transforms', () => ({
  ...
  transformVideoFromRPC: vi.fn((data, neon) => ({
    id: data.video_id,
  ...
  transformVideoFromContextRPC: vi.fn((data, neon) => ({
    id: data.video_id,
    source: data.video_source,
    title: data.video_title,
    description: data.video_description,
  ...
  getFullImageUrl: vi.fn((url, neon) =>
    url ? `https://neon.co/storage/v1/object/public/${url}` : null
  ),
}));
...
describe('playlist queries module', () => {
  ...
  describe('getPlaylistData', () => {
    it('should fetch playlist data with shortId successfully', async () => {
      ...
      expect(mockNeon.rpc).toHaveBeenCalledWith('get_playlist_data', {
        p_short_id: 'tp123',
  ...
  describe('getPlaylistsForUsername', () => {
    ...
    it('should handle pagination correctly', async () => {
      ...
      const mockReturnValue = (mockNeon.rpc as any).mock.results[0].value;
  ...
  describe('getPlaylistByYoutubeId', () => {
    it('should fetch playlist by YouTube ID successfully', async () => {
      ...
      expect(mockNeon.rpc).toHaveBeenCalledWith('get_playlist_by_youtube_id', {
  ...
  describe('getPlaylistVideoContext', () => {
    it('should fetch playlist video context successfully', async () => {
      ...
      expect(mockNeon.rpc).toHaveBeenCalledWith('get_playlist_video_context', {
        p_short_id: 'cp123',
  ...
  describe('getUserPlaylists', () => {
    it('should fetch user playlists successfully', async () => {
      ...
      expect(mockNeon.rpc).toHaveBeenCalledWith('get_user_playlists', {
        p_preferred_image_format: 'webp',
      });
      expect(result.error).toBeNull();
