...
describe('playlist duration module', () => {
  let mockNeon: NeonPostgrestClient;
  let mockFrom: any;
  let mockSelect: any;
  let mockEq: any;
  let mockIn: any;
  ...
  describe('getPlaylistTotalDuration', () => {
    it('should calculate total duration for playlist with multiple videos', async () => {
      ...
      expect(mockNeon.from).toHaveBeenCalledWith('playlist_videos');
      expect(mockSelect).toHaveBeenCalledWith('video_id');
      expect(mockEq).toHaveBeenCalledWith('playlist_id', 123);

      expect(mockNeon.from).toHaveBeenCalledWith('videos');
    ...
    it('should return zero duration for empty playlist', async () => {
      ...
      expect(mockNeon.from).toHaveBeenCalledTimes(1);
      expect(mockNeon.from).toHaveBeenCalledWith('playlist_videos');
    });
  ...
  describe('performance and reliability', () => {
    ...
    it('should handle concurrent calls correctly', async () => {
      ...
      const [result1, result2] = await Promise.all([
        getPlaylistTotalDuration({ neon: mockNeon, playlistId: 1414 }),
        getPlaylistTotalDuration({ neon: mockNeon, playlistId: 1515 }),
      ]);
