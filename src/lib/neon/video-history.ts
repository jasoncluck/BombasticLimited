...
export async function startVideoHistorySession({
  ...
  const { data, error } = await neon.rpc('start_video_history_session', {
    p_video_id: videoId,
...
export async function updateVideoHistoryEndTime({
  ...
  const { data, error } = await neon.rpc('update_video_history_end_time', {
    p_video_id: videoId,
    p_session_start_time: sessionStartTime.toISOString(),
...
export async function getUserVideoHistory({
  ...
  const { data, error } = await neon.rpc('get_user_video_history', {
    p_video_id: videoId,
    p_limit: limit,
    p_offset: offset,
  });
...
export async function getVideoAnalytics({
  ...
  const { data, error } = await neon.rpc('get_video_analytics', {
    p_video_id: videoId,
...
export class VideoWatchTimeTracker {
  ...
  private neon: NeonPostgrestClient<Database>;
