...
export function verifyWebhookSignature(
  ...
  if (!TWITCH_WEBHOOK_SECRET) {
    console.log(
      '⚠️  Skipping webhook signature verification (no secret configured)'
    );
    return true;
  }
...
export async function initializeWebhookState(): Promise<void> {
  ...
  try {
    ...
    if (currentStreams.length > 0) {
      console.log(
        `🎯 Initialized with ${currentStreams.length} live streams: ${currentStreams.join(', ')}`
      );
      subscribers.forEach((callback) => {
        try {
          callback(currentStreams);
          ...
          console.error(
            'Error notifying subscriber during initialization:',
            error
          );
        }
      });
...
export async function handleWebhookEvent(request: Request): Promise<Response> {
  try {
    ...
    if (!TWITCH_WEBHOOK_SECRET) {
      console.log(
        'ℹ️  Webhook signature verification skipped (no secret configured)'
      );
