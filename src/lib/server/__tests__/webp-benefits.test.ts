...
describe('WebP vs JPEG Compression Benefits', () => {
  it('should demonstrate significant file size reduction with WebP', async () => {
    ...
    console.log(`   Savings:   ${(jpegSize - webpSize).toLocaleString()} bytes`);
    ...
    const dataUrlReduction = ((jpegDataUrlSize - webpDataUrlSize) / jpegDataUrlSize * 100);
    
    console.log('\n📄 Data URL sizes (as stored in application):');
    console.log(`   JPEG data URL: ${jpegDataUrlSize.toLocaleString()} characters`);
    console.log(`   WebP data URL: ${webpDataUrlSize.toLocaleString()} characters`);
