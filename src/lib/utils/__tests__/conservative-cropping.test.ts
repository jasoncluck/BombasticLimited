...
import { calculateDynamicCropDimensions, validateAndAdjustCropDimensions } from '../dynamic-crop-dimensions';
...
describe('Conservative Cropping for Small Images', () => {
  describe('Problem Statement Requirements', () => {
    ...
    it('should always respect custom crop properties when provided', () => {
      ...
      const smallResult = validateAndAdjustCropDimensions(customProps, 320, 180, 'standard', customProps);
      ...
      const largeResult = validateAndAdjustCropDimensions(customProps, 1280, 720, 'maxres', customProps);
    ...
    it('should use conservative cropping for standard images without custom properties', () => {
      ...
      const defaultProps: PlaylistImageProperties = { x: 0, y: 0, width: 0, height: 0 };
      
      const result = validateAndAdjustCropDimensions(defaultProps, 320, 180, 'standard', null);
    ...
    it('should preserve user preferences for custom properties on small images', () => {
      ...
      const result = validateAndAdjustCropDimensions(userCustomProps, 320, 180, 'standard', userCustomProps);
