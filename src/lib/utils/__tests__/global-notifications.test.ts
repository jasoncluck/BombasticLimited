...
describe('global-notifications', () => {
  ...
  describe('NOTIFICATION_TEMPLATES', () => {
    it('should have welcome template with correct structure', () => {
      ...
      expect(template).toEqual({
        ...
        message: 'Thanks for being part of our community. Enjoy exploring the latest content from your favorite creators.',
    ...
    it('should have maintenance template with correct structure', () => {
      ...
      expect(template).toEqual({
        ...
        message: 'We will be performing scheduled maintenance tonight. Some features may be temporarily unavailable.',
    ...
    it('should have unique source values', () => {
      const sources = Object.values(NOTIFICATION_TEMPLATES).map(t => t.metadata.source);
  ...
  describe('sendTemplateNotification', () => {
    it('should send notification using welcome template', async () => {
      ...
      expect(mockSupabase.mockRpc).toHaveBeenCalledWith('create_notification_for_all_users', {
        ...
        notification_message: 'Thanks for being part of our community. Enjoy exploring the latest content from your favorite creators.',
    ...
    it('should send notification using maintenance template', async () => {
      ...
      const result = await sendTemplateNotification(mockSupabase, 'maintenance');

      expect(mockSupabase.mockRpc).toHaveBeenCalledWith('create_notification_for_all_users', {
        notification_type: 'system',
        notification_title: 'Scheduled Maintenance',
        notification_message: 'We will be performing scheduled maintenance tonight. Some features may be temporarily unavailable.',
    ...
    it('should apply custom message override', async () => {
      ...
      expect(mockSupabase.mockRpc).toHaveBeenCalledWith('create_notification_for_all_users', {
        notification_type: 'system',
        notification_title: 'Scheduled Maintenance',
        notification_message: 'Custom maintenance message with specific timing.',
        notification_metadata: { source: 'admin_maintenance' },
        notification_action_url: '/support/maintenance',
      });
    ...
    it('should apply all customizations simultaneously', async () => {
      ...
      const result = await sendTemplateNotification(mockSupabase, 'newFeature', {
        ...
        message: 'We have launched an incredible new feature that will change everything!',
        metadata: {
          version: '3.0.0',
  ...
  describe('integration scenarios', () => {
    it('should work with all template names', async () => {
      const templateNames = Object.keys(NOTIFICATION_TEMPLATES) as Array<keyof typeof NOTIFICATION_TEMPLATES>;
      
      for (const templateName of templateNames) {
        mockSupabase.mockRpc.mockResolvedValue({ data: 100, error: null });
        
        const result = await sendTemplateNotification(mockSupabase, templateName);
    ...
    it('should work in feature release workflow', async () => {
      ...
      const result = await sendTemplateNotification(mockSupabase, 'newFeature', {
        ...
        message: 'Switch to dark mode in your account settings for a better viewing experience.',
        metadata: {
