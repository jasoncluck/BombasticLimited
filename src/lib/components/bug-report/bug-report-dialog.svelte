<script lang="ts">
    ...
    try {
      ...
      if (error instanceof Error && 'issues' in error) {
        const zodError = error as {
          issues: Array<{
            path: Array<keyof BugReportSchema>;
            message: string;
          }>;
        };
  ...
  async function cleanupImages(keepUploaded = false): Promise<void> {
    for (const img of imageFiles) {
      ...
      // Only delete uploaded images if not keeping them (i.e., on submission failure or dialog close)
          ...
          >
            ...
            <div class="flex flex-col gap-2">
              ...
              <Textarea
                ...
                class="min-h-20 {errors.description
                  ? 'border-destructive'
                  : ''}"
                inputmode="text"
                autocomplete="off"
            ...
            <!-- Steps to Reproduce -->
            <div class="flex flex-col gap-2">
              <label for="steps" class="text-sm">Steps to Reproduce</label>
              <Textarea
                id="steps"
...
3. See error"
            ...
            <!-- Screenshots/Images -->
            <div class="flex flex-col gap-2">
              <label for="images" class="flex items-center gap-2 text-sm">
