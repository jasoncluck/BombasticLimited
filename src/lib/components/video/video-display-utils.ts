import { processVideoThumbnails, type VideoWithProcessedThumbnail } from './video-thumbnail-service';
 ...
 */
export async function processVideosForDisplay(videos: Video[]): Promise<VideoWithProcessedThumbnail[]> {
  ...
  $effect(() => {
    ...
    processVideoThumbnails(videos)
      ...
      .catch((err) => {
        ...
        processedVideos = videos.map(video => ({ ...video, processedThumbnailUrl: null }));
        isProcessing = false;
      });
  });

  return {
    get processedVideos() { return processedVideos; },
