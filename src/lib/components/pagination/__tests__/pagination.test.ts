...
describe('pagination', () => {
  ...
  describe('generatePaginationUrl', () => {
    ...
    it('should preserve other query parameters', () => {
      ...
      expect(result).toBe('https://example.com/videos?sort=name&limit=25&page=2');
    });

    it('should handle complex URLs', () => {
      const url = new URL('https://example.com/path/to/videos?sort=date&filter=new');
      const result = generatePaginationUrl({ url, pageNum: 10 });
      expect(result).toBe('https://example.com/path/to/videos?sort=date&filter=new&page=10');
    });
    ...
    it('should handle URLs with existing page and other parameters', () => {
      const url = new URL('https://example.com/videos?page=1&sort=name&limit=50');
      const result = generatePaginationUrl({ url, pageNum: 8 });
      expect(result).toBe('https://example.com/videos?page=8&sort=name&limit=50');
    });
  });
  ...
  describe('updatePaginationQueryParams', () => {
    ...
    it('should preserve existing query parameters', () => {
      ...
      expect(goto).toHaveBeenCalledWith('https://example.com/videos?sort=name&limit=25&page=2', { 
        invalidate: [] 
      });
    });
    ...
    it('should replace existing page parameter', () => {
      ...
      expect(goto).toHaveBeenCalledWith('https://example.com/videos?page=5&sort=date', { 
  ...
  describe('preloadPaginationPage', () => {
    ...
    it('should preload data for pagination URL', async () => {
      ...
      expect(preloadData).toHaveBeenCalledWith('https://example.com/videos?page=3');
    });
    ...
    it('should handle preload errors silently', async () => {
      ...
      // Should not throw
      await expect(preloadPaginationPage({ url, pageNum: 2 }))
        .resolves.toBeUndefined();
      
      expect(console.debug).toHaveBeenCalledWith('Pagination preload failed:', preloadError);
    });
    ...
    it('should preserve existing query parameters in preload URL', async () => {
      ...
      expect(preloadData).toHaveBeenCalledWith('https://example.com/videos?sort=date&limit=50&page=5');
    });
    ...
    it('should handle different page numbers', async () => {
      ...
      expect(preloadData).toHaveBeenCalledWith('https://example.com/videos?page=1');
      
      await preloadPaginationPage({ url, pageNum: 10 });
      expect(preloadData).toHaveBeenCalledWith('https://example.com/videos?page=10');
      
      await preloadPaginationPage({ url, pageNum: 0 });
      expect(preloadData).toHaveBeenCalledWith('https://example.com/videos?page=0');
    });
    ...
    it('should handle complex URLs', async () => {
      ...
      expect(preloadData).toHaveBeenCalledWith('https://example.com/path/to/content?filter=active&search=test&page=7');
    });
  });
  ...
  describe('integration scenarios', () => {
    it('should handle complete pagination workflow', () => {
      ...
      const totalPages = getNumberOfPages({ count: totalItems, perPage: itemsPerPage });
      expect(totalPages).toBe(10);
      
      // 3. Generate URLs for navigation
      const baseUrl = new URL('https://example.com/videos?sort=name');
      const nextPageUrl = generatePaginationUrl({ url: baseUrl, pageNum: currentPage + 1 });
      const prevPageUrl = generatePaginationUrl({ url: baseUrl, pageNum: currentPage - 1 });
    ...
    it('should handle edge cases in pagination workflow', () => {
      ...
      const lastPageUrl = generatePaginationUrl({ url: baseUrl, pageNum: totalPages });
    ...
    it('should maintain URL consistency across operations', () => {
      const originalUrl = new URL('https://example.com/videos?sort=date&filter=active');
      
      // Extract page number (should default to 1)
      const currentPage = getPaginationQueryParams({ searchParams: originalUrl.searchParams });
      expect(currentPage).toBe(1);
      
      // Generate new URL with different page
      const newPageUrl = generatePaginationUrl({ url: originalUrl, pageNum: 5 });
      expect(newPageUrl).toBe('https://example.com/videos?sort=date&filter=active&page=5');
      
      // Parse the new URL to verify page extraction
      const newUrl = new URL(newPageUrl);
      const extractedPage = getPaginationQueryParams({ searchParams: newUrl.searchParams });
