<script lang="ts">
  ...
  import { EditIcon, ListVideo } from "@lucide/svelte";
  ...
  const playlistForm = superForm(form, {
    ...
    dataType: "json",
    onSubmit() {
      // Update drawer state to show loading
      drawerState.updateFormState(true);
    },
    onResult(event) {
      if (event.result.type !== "success") {
        drawerState.updateFormState(false);
      }
    },
    async onUpdated(event) {
      if (event.form.valid) {
        const { isDeletingPlaylistImage, ...data } = event.form.data;
        console.log("calling drawer state");
        drawerState.toggle();
        drawerState.updateFormState(false);
        playlistForm.reset();
...
<!-- Remove the Footer wrapper, just content -->
          ...
          <Popover.Root>
            ...
            <Popover.Content align="start"
              ><p class="text-sm">
                ...
                selecting a video thumbnail to use as the image. This can be set
                by right-clicking on a video in a playlist.
              </p>
            </Popover.Content>
          </Popover.Root>
        {/if}
