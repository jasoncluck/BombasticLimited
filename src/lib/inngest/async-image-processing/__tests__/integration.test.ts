...
describe('Image Processing Job Duplication Fix - Integration', () => {
  ...
  it('should validate database migration adds required columns and functions', () => {
    ...
    const workerSpecificFunctions = requiredFunctions.filter(func => func.includes('with_worker'));
  ...
  it('should ensure concurrency control prevents duplicate processing', () => {
    ...
    const job1 = { jobId: 'job-123', entityType: 'video', entityId: 'video-1', imageType: 'thumbnail' };
    const job2 = { jobId: 'job-456', entityType: 'video', entityId: 'video-1', imageType: 'thumbnail' };
