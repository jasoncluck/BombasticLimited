...
describe('Enhanced Job Poller', () => {
  ...
  describe('Configuration', () => {
    it('should be defined with correct configuration', () => {
      ...
      expect(pollPendingJobs.retries).toBe(3);
      expect(pollPendingJobs.concurrency?.limit).toBe(1);
    });
  });
  ...
  describe('Stale Job Cleanup', () => {
    it('should call cleanup_stale_processing_jobs with 10-minute threshold', async () => {
      ...
      // Execute the handler
      await (pollPendingJobs as any).handler({ step: mockStep });
  ...
  describe('Enhanced Diagnostics', () => {
    it('should fetch job status counts for diagnostics', async () => {
      ...
      const result = await (pollPendingJobs as any).handler({ step: mockStep });
