...
export const pollPendingJobs = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    console.log(`🔄 [${pollStartTimestamp}] Worker ${WORKER_ID} starting image processing job polling cycle...`);

    try {
      // Step 1: Get jobs atomically using the new enhanced function
      const pendingJobs = await step.run('query-pending-jobs-atomic', async () => {
        console.log(`📋 [${new Date().toISOString()}] Worker ${WORKER_ID} querying for pending jobs using atomic method...`);
        ...
        try {
          ...
          for (let i = 0; i < MAX_JOBS_PER_POLL; i++) {
            ...
            console.log(`🔍 [${new Date().toISOString()}] Worker ${WORKER_ID} attempting atomic job lock ${i + 1}/${MAX_JOBS_PER_POLL}...`);
            ...
            console.log(`⏱️ [${new Date().toISOString()}] Worker ${WORKER_ID} atomic query ${i + 1} completed in ${queryDuration}ms`);

            if (error) {
              console.error(`❌ [${new Date().toISOString()}] Worker ${WORKER_ID} database query failed on attempt ${i + 1}:`, error);
            ...
            if (!data || (Array.isArray(data) && data.length === 0)) {
              ...
              console.log(`ℹ️ [${new Date().toISOString()}] Worker ${WORKER_ID} no job returned on atomic query ${i + 1} - all jobs processed`);
              break;
            ...
            if (job) {
              ...
              console.log(`✅ [${new Date().toISOString()}] Worker ${WORKER_ID} ATOMICALLY locked job ${job.job_id} for ${job.entity_type}/${job.entity_id}/${job.image_type} (attempt ${job.attempts}/3, started: ${job.processing_started_at})`);
            } else {
              console.log(`⚠️ [${new Date().toISOString()}] Worker ${WORKER_ID} empty job object returned on atomic query ${i + 1}`);
              break;
          ...
          console.log(`📊 [${new Date().toISOString()}] Worker ${WORKER_ID} atomic job discovery completed: ${jobs.length} jobs locked in ${totalQueryTime}ms`);
          
          // Log job details for debugging
          jobs.forEach((job, index) => {
            console.log(`📋 [${new Date().toISOString()}] Worker ${WORKER_ID} Job ${index + 1}: ${job.job_id} (${job.entity_type}/${job.entity_id}/${job.image_type}, attempts: ${job.attempts}, started: ${job.processing_started_at})`);
          ...
          console.error(`❌ [${new Date().toISOString()}] Worker ${WORKER_ID} query error during atomic job polling:`, queryError);
      ...
      if (jobsPolled === 0) {
        ...
        console.log(`✅ [${new Date().toISOString()}] Worker ${WORKER_ID} no pending jobs found - polling cycle complete in ${duration}ms`);
      ...
      const results = await step.run('send-processing-events', async () => {
        ...
        for (const [index, job] of pendingJobs.entries()) {
          ...
          try {
            ...
            console.log(`📤 [${new Date().toISOString()}] Worker ${WORKER_ID} sending processing event ${index + 1}/${jobsPolled} for ${job.entity_type} ${job.entity_id} (${job.image_type}) - Job ID: ${job.job_id}`);
            ...
            console.log(`✅ [${new Date().toISOString()}] Worker ${WORKER_ID} successfully queued processing for ${job.entity_type} ${job.entity_id} with job ID ${job.job_id} in ${jobDuration}ms`);
          } catch (sendError) {
            const jobDuration = Date.now() - jobStartTime;
            
            console.error(`❌ [${new Date().toISOString()}] Worker ${WORKER_ID} failed to send processing event for ${job.entity_type} ${job.entity_id} after ${jobDuration}ms:`, sendError);
      ...
      console.log(`🎯 [${new Date().toISOString()}] Worker ${WORKER_ID} polling cycle completed: ${jobsSent}/${jobsPolled} jobs successfully sent, ${errors} errors, ${duration}ms total`);
      ...
      results.forEach((result, index) => {
        ...
        console.log(`${status} [${new Date().toISOString()}] Worker ${WORKER_ID} Job ${index + 1} result: ${result.entityType}/${result.entityId} (${result.jobId}) - ${result.duration}ms${errorMsg ? ` - Error: ${errorMsg}` : ''}`);
      ...
      console.error(`❌ [${new Date().toISOString()}] Worker ${WORKER_ID} job polling cycle failed after ${duration}ms:`, error);
