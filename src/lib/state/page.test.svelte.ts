...
describe("PageStateClass", () => {
	...
	describe("viewport reference setters", () => {
		it("should set sidebar viewport reference", () => {
			...
			expect(pageState.viewportRefs.sidebarViewportRef).toStrictEqual(mockViewportRef);
		});

		it("should set content viewport reference", () => {
			pageState.setContentViewportRef(mockViewportRef);
			expect(pageState.viewportRefs.contentViewportRef).toStrictEqual(mockViewportRef);
		});
	...
	describe("viewport drag over handling", () => {
		...
		it("should start scrolling up when in top scroll zone", () => {
			...
			const mockScrollState: ScrollState = { scrolling: false, direction: null, interval: null };

			pageState.handleViewportDragOver(mockEvent, mockViewportRef, mockScrollState);
...
describe("context functions", () => {
	it("should set page state context", async () => {
		...
		setPageState();
		expect(setContext).toHaveBeenCalledWith("$_page_state", expect.any(PageStateClass));
	});

	it("should set page state context with custom key", async () => {
		const { setContext } = vi.mocked(await import("svelte"));
		setPageState("custom_key");
		expect(setContext).toHaveBeenCalledWith("custom_key", expect.any(PageStateClass));
	});
