...
describe('YouTube Playlist Sync Image Processing Fix', () => {
  it('should queue image processing jobs for playlist thumbnails during sync', () => {
    ...
    const mockQueueFunction = vi.fn().mockImplementation(
      (supabaseClient: any, playlistId: number, thumbnailUrl: string | null, priority: number) => {
        if (!thumbnailUrl) {
          console.log(`No thumbnail URL provided for playlist ${playlistId}, skipping image processing`);
    ...
    expect(mockQueueFunction).toHaveBeenCalledWith(null, playlistId, thumbnailUrl, priority);
  ...
  it('should validate the expected flow: YouTube URL -> Image Processing -> Storage Upload -> Database Path Storage', () => {
    ...
    const expectedFlow = {
      ...
      step2: 'Queue image processing job via queuePlaylistThumbnailProcessing()',
      step3: 'Async system processes: download -> crop -> upload to storage',
      step4: 'Database updated via complete_image_processing_job_with_worker RPC',
      result: 'Playlist has image_webp_url pointing to storage path instead of YouTube URL'
    };
  ...
  it('should not break playlist sync if image processing fails', () => {
    ...
    const mockQueueFunctionWithError = vi.fn().mockImplementation(
      (supabaseClient: any, playlistId: number, thumbnailUrl: string | null, priority: number) => {
        try {
          ...
          console.error('Image processing failed but not breaking sync:', error);
          return Promise.resolve();
        }
      }
    );
    ...
    expect(() => {
      mockQueueFunctionWithError(null, 123, 'https://example.com/thumb.jpg', 50);
    }).not.toThrow();
  });
});