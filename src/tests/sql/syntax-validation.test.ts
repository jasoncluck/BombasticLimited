...
import { discoverSqlFiles, getMigrationFiles } from '$lib/test-utils/sql-discovery';
...
describe('SQL Syntax Validation', () => {
  ...
  test('should discover SQL files in supabase directory', async () => {
    ...
    const orders = migrationFiles.map(f => f.order).filter(o => o !== undefined);
  ...
  test('should validate syntax of all SQL files', async () => {
    ...
    for (const file of files) {
      try {
        ...
        if (parenDiff > 5) { // Allow small difference due to complex SQL formatting
          invalidFiles.push({
            file: file.name,
        ...
        const hasValidSqlKeywords = /\b(CREATE|ALTER|DROP|INSERT|UPDATE|DELETE|SELECT|SET|GRANT|REVOKE)\b/i.test(content);
    ...
    if (invalidFiles.length > 0) {
      const errorDetails = invalidFiles.map(f => `${f.file}: ${f.error}`).join('\n');
  ...
  test('should validate migration files have proper structure', async () => {
    ...
    for (const migration of migrationFiles) {
      ...
      const hasDescriptiveComment = content.match(/--.*(?:[Mm]igration|[Pp]urpose|[Ff]unctions?|[Cc]reate)/);
      if (!hasDescriptiveComment) {
        console.warn(`Migration file ${migration.name} missing descriptive comment`);
      }
      
      // Should have proper PostgreSQL settings (if any SET statements exist)
      if (content.includes('SET ') && content.match(/SET.*(?:statement_timeout|lock_timeout|client_encoding|default_tablespace|search_path)/)) {
        // This is good - has PostgreSQL configuration
        expect(content).toMatch(/SET.*(?:statement_timeout|lock_timeout|client_encoding|default_tablespace|search_path)/);
      }
  ...
  test('should check for SQL anti-patterns', async () => {
    const files = await discoverSqlFiles(SUPABASE_DIR);
    const antiPatterns: Array<{ file: string; pattern: string; line?: number }> = [];
    ...
    for (const file of files) {
      ...
      lines.forEach((line, index) => {
        ...
        if ((/UPDATE\s+\w+\s+SET/i.test(line) || /DELETE\s+FROM\s+\w+$/i.test(line)) && 
            !line.includes('WHERE') && !line.includes('--')) {
          antiPatterns.push({
            file: file.name,
