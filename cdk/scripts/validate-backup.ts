import { BackupEvent, BackupResult, handler } from "../lib/lambda/database-backup";
...
async function validateBackupInfrastructure() {
  ...
  if (process.env.SUPABASE_SERVICE_API_KEY_PROD?.startsWith("eyJ")) {
    try {
      ...
      console.log(`⚠️  Dry run test skipped: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  } else {
    console.log("⚠️  Dry run test skipped: Real Supabase credentials not available");
