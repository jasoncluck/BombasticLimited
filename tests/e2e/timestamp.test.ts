...
authenticatedTest.describe('Video Timestamp and Continue Watching UI', () => {
    ...
    async ({ authenticatedPage }) => {
      ...
      if (continueWatchingSection) {
        ...
        if (continueVideos.length > 0) {
          ...
          const hasProgress = await videoHelpers.hasProgressIndicator(continueVideos[0]);
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage }) => {
      ...
      if (videoTitle) {
        const titleElement = authenticatedPage.locator('h1, h2, h3, p').filter({ hasText: videoTitle });
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage }) => {
      ...
      if (continueWatchingSection) {
        ...
        if (continueVideos.length > 0) {
          const videoTitle = await videoHelpers.getVideoTitle(continueVideos[0]);
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage }) => {
      ...
      if (contextMenu) {
        ...
        const watchedOption = contextMenu.locator('text=watched').or(
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage }) => {
      ...
      if (dropdownMenu) {
        ...
        const options = await dropdownMenu.locator('button, [role="menuitem"], a').all();
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage }) => {
      try {
        ...
        const bulkOptions = authenticatedPage.locator('text=videos').or(
          authenticatedPage.locator('text=selected').and(authenticatedPage.locator('text=items'))
        );
...
unauthenticatedTest.describe('Unauthenticated User Timestamp Limitations', () => {
  let videoHelpers: VideoHelpers;
  ...
  unauthenticatedTest(
    ...
    async ({ unauthenticatedPage }) => {
      ...
      if (contextMenu) {
        ...
        const timestampOptions = contextMenu.locator('text=watched').or(
          contextMenu.locator('text=progress').and(contextMenu.locator('text=reset'))
        );
