...
authTest.describe('Homepage Authenticated Tests', () => {
  authTest(
  ...
  // This test will only currently pass if the images exist which may be a problem in local dev
  authTest(
    ...
    async ({ authenticatedPage: page }) => {
      ...
      const secondContentCard = page.getByTestId('content-card').nth(1);
      await secondContentCard.click();
