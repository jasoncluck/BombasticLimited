...
export interface VideoHelpers {
  // Basic video operations
  getFirstVideoCard(): Promise<Locator>;
  getVideoTitle(videoCard: Locator): Promise<string | null>;
  navigateToVideo(videoCard: Locator): Promise<Locator>;
  
  // Continue watching functionality
  getContinueWatchingSection(): Promise<Locator | null>;
  getContinueWatchingVideos(): Promise<Locator[]>;
  hasProgressIndicator(videoCard: Locator): Promise<boolean>;
  isVideoWatched(videoCard: Locator): Promise<boolean>;
  
  // Context menu and dropdown operations
  openContextMenu(videoCard: Locator): Promise<Locator | null>;
  openContentDropdown(videoCard: Locator): Promise<Locator | null>;
  markVideoAsWatched(videoCard: Locator): Promise<boolean>;
  resetVideoProgress(videoCard: Locator): Promise<boolean>;
  
  // Multi-selection operations
  multiSelectVideos(indices: number[], useShift?: boolean): Promise<Locator>;
  getSelectionIndicators(): Promise<Locator>;
  
  // View mode operations
  switchToCardView(): Promise<void>;
  switchToTableView(): Promise<void>;
  getCurrentViewMode(): Promise<ViewMode>;
  switchViewMode(): Promise<ViewMode>;
  
  // Navigation and utility
  goToHomepage(): Promise<void>;
  waitForOperation(timeoutMs?: number): Promise<void>;
  getVideoCards(): Promise<Locator[]>;
}
...
function createVideoHelpers(page: Page): VideoHelpers {
  return {
    /**
     * Get the first available video card from the carousel
     */
    async getFirstVideoCard(): Promise<Locator> {
      const videoCard = page.getByTestId('carousel-item').first();
    ...
    async multiSelectVideos(
      ...
      if (currentMode === 'table') {
        ...
        videoItems = page.locator('tr[data-testid*="video"], tbody tr').filter({ hasText: /.+/ });
      } else {
        // In card/carousel mode, use carousel items
        videoItems = page.getByTestId('carousel-item');
      }
    ...
    async getSelectionIndicators(): Promise<Locator> {
      ...
      const selectionIndicators = page
        .locator('.selected, [data-selected="true"], [aria-selected="true"], .bg-primary, .bg-accent')
        .or(page.locator('tr.selected, tr[data-selected="true"], tr[aria-selected="true"]'))
        .or(page.locator('[class*="selected"], [class*="highlight"]'));
    ...
    async switchToCardView(): Promise<void> {
      ...
      const userPreferences = page.getByTestId('user-preferences');
      ...
      if (await userPreferences.isVisible({ timeout: 3000 })) {
        ...
        const cardOption = page.locator('[role="menuitem"]').filter({ hasText: /Card/i });
    ...
    async switchToTableView(): Promise<void> {
      ...
      if (await userPreferences.isVisible({ timeout: 3000 })) {
        ...
        const tableOption = page.locator('[role="menuitem"]').filter({ hasText: /Table/i });
...
// Video test that extends authenticated test
export const videoTest = authenticatedTest.extend<VideoFixtures>({
  videoHelpers: async ({ authenticatedPage }, use) => {
    const helpers = createVideoHelpers(authenticatedPage);
    await use(helpers);
    // No cleanup needed for video helpers - they don't create persistent state
  },

  videoPage: async ({ authenticatedPage }, use) => {
    // Navigate to a page where videos can be managed
