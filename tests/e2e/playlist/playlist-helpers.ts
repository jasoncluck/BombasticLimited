...
export interface PlaylistHelpers {
  ...
  changePlaylistSortOrder(
    sortKey: 'Custom' | 'Published At' | 'Title',
...
export function createPlaylistHelpers(page: Page): PlaylistHelpers {
  return {
    ...
    async changePlaylistSortOrder(
      sortKey: 'Custom' | 'Published At' | 'Title',
      order?: 'ascending' | 'descending'
    ): Promise<void> {
      // Look for sort dropdown or filter controls
      const sortDropdown =
        page.getByTestId('content-filter') ||
    ...
    async getCurrentSortOrder(): Promise<{ key: string; order: string }> {
      ...
      const sortKey =
        url.searchParams.get('playlistOrder') ||
        url.searchParams.get('datePublished') ||
        url.searchParams.get('title') ||
        'Custom';
      const order =
        url.searchParams.get(
          sortKey === 'Custom' ? 'playlistOrder' : sortKey
        ) || 'ascending';
    ...
    async verifyDragDisabled(): Promise<void> {
      ...
      if ((await videoCards.count()) > 1) {
        ...
        expect(
          Math.abs((initialPosition?.x || 0) - (newPosition?.x || 0))
        ).toBeLessThan(10);
      }
    },
    ...
    async resumePlaylistFromContinueWatching(
      videoCard: Locator
    ): Promise<void> {
