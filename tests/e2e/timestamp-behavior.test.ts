...
authenticatedTest.describe('Timestamp Behavior Scenarios', () => {
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage, testDataManager, testUser }) => {
      ...
      const continueWatchingSection = await videoHelpers.getContinueWatchingSection();
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage, testDataManager, testUser }) => {
      ...
      const initialContinueSection = await videoHelpers.getContinueWatchingSection();
      ...
      const finalContinueSection = await videoHelpers.getContinueWatchingSection();
      
      if (finalContinueSection) {
        const finalContinueVideos = await videoHelpers.getContinueWatchingVideos();
        ...
        if (finalCount > initialCount) {
          const newVideoTitles = await Promise.all(
            finalContinueVideos.slice(initialCount).map(video => videoHelpers.getVideoTitle(video))
          );
          
          const ourVideoAdded = newVideoTitles.some(title => title === videoTitle);
    ...
    async ({ authenticatedPage, testDataManager, testUser }) => {
      ...
      if (continueWatchingSection) {
        ...
        if (continueVideoTitles.includes(videoTitle)) {
          ...
          const wasReset = await videoHelpers.resetVideoProgress(videoCardAgain);
          
          if (wasReset) {
            await videoHelpers.waitForOperation(2000);
            
            // Check if video was removed from continue watching
            const updatedContinueVideos = await videoHelpers.getContinueWatchingVideos();
            const updatedTitles = await Promise.all(
              updatedContinueVideos.map(video => videoHelpers.getVideoTitle(video))
            );
  ...
  authenticatedTest(
    ...
    async ({ authenticatedPage }) => {
      ...
      if (initialContinueSection) {
        ...
        const reloadedContinueSection = await videoHelpers.getContinueWatchingSection();
      ...
      if (continueWatchingSection) {
        ...
        if (continueVideos.length > 1) {
          ...
          const finalTitles = await Promise.all(
            finalContinueVideos.map(video => videoHelpers.getVideoTitle(video))
          );
