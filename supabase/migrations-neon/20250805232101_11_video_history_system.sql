...
ADD CONSTRAINT "video_history_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles" ("id") ON DELETE CASCADE;
...
SET
  search_path = '' AS $$
  SELECT
    (auth.user_id())::uuid::text || '|' || vh.video_id || '|' || EXTRACT(EPOCH FROM vh.session_start_time)::bigint::text AS id,
    vh.video_id,
    vh.source,
    vh.seconds_watched,
    vh.session_start_time,
  ...
  WHERE vh.user_id = (auth.user_id())::uuid
