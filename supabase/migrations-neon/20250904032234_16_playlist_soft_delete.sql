...
SET
  ...
  WHERE up.user_id = (auth.user_id())::uuid
...
BEGIN
  ...
  WITH playlist_data AS (
    ...
    -- FIXED: Use auth.user_id() instead of p_user_id parameter
    LEFT JOIN public.user_playlists up ON up.id = p.id AND up.user_id = (auth.user_id())::uuid
  ...
  JOIN public.videos v ON pv.video_id = v.id
  -- FIXED: Use auth.user_id() instead of p_user_id parameter
  LEFT JOIN public.timestamps t ON v.id = t.video_id AND t.user_id = (auth.user_id())::uuid
...
BEGIN
    -- Get current user if not provided
    target_user_id := COALESCE(p_user_id, (auth.user_id())::uuid);
...
-- handle_user_deletion_cleanup() + the on_auth_user_deletion BEFORE DELETE
-- trigger on "auth"."users" removed (Neon adaptation): auth.users doesn't
-- exist on Neon, and this duplicated the playlist-cleanup logic already
-- handled directly (not via trigger) by the adapted public.delete_user()
-- in 08b_user_lifecycle_functions.sql. This file no longer redefines
