...
SELECT ok(
    EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'videos' AND table_schema = 'public'),
    'videos table should exist for testing'
);

SELECT ok(
    EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'playlists' AND table_schema = 'public'),
    'playlists table should exist for testing'
);

SELECT ok(
    EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'timestamps' AND table_schema = 'public'),
...
SELECT ok(
    EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'image_processing_jobs' AND table_schema = 'public'),
    'image_processing_jobs table should exist for testing'
);

-- Verify that we have functions to test
SELECT ok(
    (SELECT COUNT(*) FROM information_schema.routines WHERE routine_schema = 'public' AND routine_type = 'FUNCTION') >= 60,
    'Should have at least 60 public functions to test'
);

-- Verify that all test files exist
SELECT ok(
    EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pg_proc'),
