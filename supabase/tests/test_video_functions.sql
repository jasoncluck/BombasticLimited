...
VALUES 
  ('test_video_1', 'YouTube', 'Test Video One', 'This is a test video about cats', 'https://example.com/thumb1.jpg', '2023-01-01 10:00:00+00', 'PT5M30S', FALSE),
  ('test_video_2', 'YouTube', 'Another Test Video', 'This video discusses dogs and puppies', 'https://example.com/thumb2.jpg', '2023-01-02 11:00:00+00', 'PT10M15S', FALSE),
  ('test_video_3', 'YouTube', 'Programming Tutorial', 'Learn JavaScript basics', 'https://example.com/thumb3.jpg', '2023-01-03 12:00:00+00', 'PT15M45S', FALSE),
  ('pending_video', 'YouTube', 'Pending Delete Video', 'This video is marked for deletion', 'https://example.com/thumb4.jpg', '2023-01-04 13:00:00+00', 'PT2M30S', TRUE)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO public.timestamps (user_id, video_id, playlist_id, video_start_seconds, watched_at, updated_at)
VALUES 
  ('44444444-4444-4444-4444-444444444444'::uuid, 'test_video_1', 1001, 120.5, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '1 hour'),
  ('44444444-4444-4444-4444-444444444444'::uuid, 'test_video_2', 1001, 0, NOW() - INTERVAL '2 hours', NOW() - INTERVAL '2 hours'),
  ('44444444-4444-4444-4444-444444444444'::uuid, 'test_video_3', 1001, 300.25, NOW() - INTERVAL '30 minutes', NOW() - INTERVAL '30 minutes')
...
INSERT INTO temp_test_user VALUES ('44444444-4444-4444-4444-444444444444'::uuid);
...
SELECT ok(
  EXISTS(SELECT 1 FROM pg_proc WHERE proname = 'get_videos_with_timestamps'),
...
SELECT ok(
  (SELECT COUNT(*) FROM public.get_videos_with_timestamps()) >= 0,
...
SELECT ok(
  NOT EXISTS(
    SELECT 1 FROM public.get_videos_with_timestamps()
    WHERE id = 'pending_video'
  ),
...
SELECT ok(
  (SELECT COUNT(column_name) FROM information_schema.columns 
   ...
   AND table_name LIKE '%get_videos_with_timestamps%') >= 10,
...
SELECT ok(
  EXISTS(SELECT 1 FROM pg_proc WHERE proname = 'search_videos'),
  'search_videos function exists'
);

-- Test search with valid term
SELECT ok(
  (SELECT COUNT(*) FROM public.search_videos('test', 0)) > 0,
...
SELECT is(
  (SELECT COUNT(*) FROM public.search_videos('', 0)),
  0::bigint,
...
SELECT is(
  (SELECT COUNT(*) FROM public.search_videos(NULL, 0)),
  0::bigint,
...
SELECT ok(
  (SELECT COUNT(*) FROM public.search_videos('test', 1)) >= 0,
...
-- Test function exists
SELECT ok(
  EXISTS(SELECT 1 FROM pg_proc WHERE proname = 'get_in_progress_videos_with_timestamps'),
...
SELECT ok(
  (SELECT COUNT(*) FROM public.get_in_progress_videos_with_timestamps()) >= 0,
...
SELECT ok(
  (SELECT COUNT(column_name) FROM information_schema.columns 
   WHERE table_schema = 'pg_temp' 
   AND table_name LIKE '%get_in_progress_videos_with_timestamps%') >= 15,
...
SELECT ok(
  (SELECT COUNT(*) FROM public.videos WHERE title IS NOT NULL AND id LIKE 'test_video_%') = 3,
  'Test videos created with proper titles'
);

-- Test duration format
SELECT ok(
  (SELECT COUNT(*) FROM public.videos WHERE duration ~ '^PT\d+[MS]' AND id LIKE 'test_video_%') >= 1,
...
-- Clean up test data
DELETE FROM public.timestamps WHERE user_id = '44444444-4444-4444-4444-444444444444'::uuid;
DELETE FROM public.playlists WHERE id = 1001;
DELETE FROM public.videos WHERE id IN ('test_video_1', 'test_video_2', 'test_video_3', 'pending_video');
DELETE FROM public.profiles WHERE id = '44444444-4444-4444-4444-444444444444'::uuid;
