...
-- Create test videos
INSERT INTO
  public.videos (id, source, title, published_at, pending_delete)
VALUES
  (
    'pl_test_1',
    'test',
...
PERFORM public.update_playlist_videos_positions (
  9999,
  ARRAY['pl_test_1', 'pl_test_2', 'pl_test_4'],
  1
);
    ...
    NOT EXISTS (
      ...
      FROM
        (
          SELECT
            video_position,
            video_position - LAG(video_position, 1, 0) OVER (
              ORDER BY
                video_position
            ) AS gap
          FROM
...
PERFORM public.update_playlist_videos_positions (
  9999,
  ARRAY['pl_test_2', 'pl_test_6', 'pl_test_7'],
  3
);
