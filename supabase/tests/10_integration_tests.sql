...
VALUES 
    ('integration_video_1', 'giantbomb', 'Integration Test Video One', 'First video for integration testing with searchable content', 'https://example.com/1.jpg', '2023-01-01'::timestamptz, false),
    ('integration_video_2', 'jeffgerstmann', 'Integration Test Video Two', 'Second video for testing search functionality', 'https://example.com/2.jpg', '2023-01-02'::timestamptz, false),
    ('integration_video_3', 'nextlander', 'Integration Test Video Three', 'Third video with different source', 'https://example.com/3.jpg', '2023-01-03'::timestamptz, false);

-- Test that search vectors were automatically created
SELECT ok(
    (SELECT search_vector FROM public.videos WHERE id = 'integration_video_1') IS NOT NULL,
...
SELECT ok(
    EXISTS(
        SELECT 1 FROM public.search_videos('Integration', 0)
        WHERE id = 'integration_video_1'
    ),
...
SELECT ok(
    EXISTS(
        SELECT 1 FROM public.search_videos('searchable', 0)
        WHERE id = 'integration_video_1'
    ),
...
INSERT INTO public.playlists (id, name, created_by, type, description)
VALUES 
    (5001, 'Integration Test Playlist', '77777777-7777-7777-7777-777777777777', 'Public', 'A comprehensive test playlist');

-- Test that playlist short_ids were automatically generated
SELECT ok(
    (SELECT short_id FROM public.playlists WHERE id = 5001) IS NOT NULL,
    'Playlist short_id should be automatically generated'
);

SELECT ok(
    length((SELECT short_id FROM public.playlists WHERE id = 5001)) > 0,
...
SELECT ok(
    (SELECT COUNT(DISTINCT source) FROM public.search_videos('Integration', 0)) >= 2,
...
SELECT ok(
    (
        ...
        SELECT COUNT(*) FROM public.playlists WHERE id = 5001
    ) = 1 AND (
