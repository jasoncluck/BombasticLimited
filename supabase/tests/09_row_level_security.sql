...
SELECT ok(
    (SELECT relrowsecurity FROM pg_class WHERE relname = 'playlists') = true,
    'RLS should be enabled on playlists table'
);

SELECT ok(
    (SELECT relrowsecurity FROM pg_class WHERE relname = 'timestamps') = true,
    'RLS should be enabled on timestamps table'
);

SELECT ok(
    (SELECT relrowsecurity FROM pg_class WHERE relname = 'user_playlists') = true,
...
SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'playlists'),
    'RLS policies should exist for playlists table'
);

SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'timestamps'),
    'RLS policies should exist for timestamps table'
);

SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'user_playlists'),
    'RLS policies should exist for user_playlists table'
);

SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'videos'),
    'RLS policies should exist for videos table'
);

SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'playlist_videos'),
    'RLS policies should exist for playlist_videos table'
);

-- Test that we have policies for different operations
SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'playlists' AND cmd = 'SELECT'),
    'Playlists should have SELECT policies'
);

SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'playlists' AND cmd = 'INSERT'),
    'Playlists should have INSERT policies'
);

SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'timestamps' AND cmd = 'SELECT'),
    'Timestamps should have SELECT policies'
);

SELECT ok(
    EXISTS(SELECT 1 FROM pg_policies WHERE tablename = 'timestamps' AND cmd = 'INSERT'),
    'Timestamps should have INSERT policies'
);

-- Test that we have a reasonable number of policies
SELECT ok(
    (SELECT COUNT(*) FROM pg_policies WHERE tablename IN ('profiles', 'playlists', 'timestamps', 'videos', 'playlist_videos', 'user_playlists')) >= 10,
