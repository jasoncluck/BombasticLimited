...
function isJobReadyForRetry(job: PendingJobRow): boolean {
  ...
  console.log(
    `Job ${job.id} (attempt ${job.attempts}) updated at ${job.updated_at}, ${minutesElapsed} minutes ago, ready: ${isReady} (cooldown: ${RETRY_COOLDOWN_MINUTES}min)`
  );
...
async function processImageJobs(): Promise<ApiResponse> {
  ...
  try {
    ...
    // 6. Filter jobs based on eligibility and retry cooldown
    ...
    console.log(
      `${eligibleJobs.length} jobs are eligible for processing, ${skippedJobs} jobs skipped (cooldown/max attempts)`
    );
    ...
    if (eligibleJobs.length === 0) {
      return {
        ...
        skipped: skippedJobs,
        queueStatus,
        message: `No jobs ready for processing (${skippedJobs} jobs in cooldown or exceeded max attempts)`,
      };
    }
    ...
    if (skippedJobs > 0) {
      message += `, ${skippedJobs} jobs skipped (cooldown/max attempts)`;
    }
