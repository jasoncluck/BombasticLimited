...
CREATE OR REPLACE FUNCTION public.duration_to_seconds (duration_text text) RETURNS integer AS $$
DECLARE
  hours integer := 0;
  minutes integer := 0;
  seconds integer := 0;
...
CREATE OR REPLACE FUNCTION public.calculate_playlist_duration (playlist_id_param bigint) RETURNS integer AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.update_playlist_duration (playlist_id_param bigint) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.trigger_update_playlist_duration_from_videos () RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.trigger_update_affected_playlist_durations () RETURNS TRIGGER AS $$
BEGIN
...
UPDATE public.playlists
SET
  duration_seconds = public.calculate_playlist_duration (id)
WHERE
  duration_seconds = 0;
