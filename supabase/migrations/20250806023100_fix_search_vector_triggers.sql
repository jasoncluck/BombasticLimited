...
CREATE OR REPLACE FUNCTION "public"."set_short_id" () RETURNS "trigger" LANGUAGE "plpgsql" AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION "public"."set_playlist_search_vector" () RETURNS "trigger" LANGUAGE "plpgsql" AS $$BEGIN
  NEW.search_vector := pg_catalog.to_tsvector('english', NEW.name);
  RETURN NEW;
END;$$;

CREATE OR REPLACE FUNCTION "public"."set_video_search_vector" () RETURNS "trigger" LANGUAGE "plpgsql" AS $$BEGIN
  NEW.search_vector := 
