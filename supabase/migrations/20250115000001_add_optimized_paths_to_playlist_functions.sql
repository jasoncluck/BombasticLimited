-- Migration: Add optimized image paths to playlist query functions
-- This migration updates RPC functions to include optimized image storage paths
-- Required for proper playlist image fallback functionality

-- Update get_playlists_for_username function to include optimized image paths
...
-- Update search_playlists function to include optimized image paths
...
BEGIN
    ...
    SELECT 
        ...
        rp.thumbnail_webp_path,
        rp.thumbnail_avif_path,
        rp.thumbnail_maxres_webp_path,
        rp.thumbnail_maxres_avif_path,
...
-- Update get_followed_playlists function to include optimized image paths
...
-- Update get_playlist_data function to include optimized image paths for playlist
CREATE OR REPLACE FUNCTION public.get_playlist_data (
  ...
  -- Playlist data with optimized image paths
  playlist_id bigint,
...
BEGIN
  ...
  WITH sorted_videos AS (
    SELECT 
      ...
      -- Add sort helpers
      CASE 
        WHEN effective_sort_key = 'video_position' THEN pv.position::text
