...
CREATE POLICY "Enable insert for users based on created_by" ON "public"."playlists" 
FOR INSERT 
TO "authenticated" 
...
CREATE POLICY "Allow read access for public playlists and owned playlists" ON "public"."playlist_videos"
FOR SELECT
...
CREATE POLICY "Allow authenticated users to insert playlist videos into their own playlists" ON "public"."playlist_videos"
FOR INSERT
TO authenticated
WITH CHECK (
  EXISTS (
...
CREATE POLICY "Allow authenticated users to delete playlist videos from their own playlists" ON "public"."playlist_videos"
FOR DELETE
TO authenticated
USING (
  EXISTS (
...
CREATE POLICY "Authenticated users can insert their own video timestamps" ON "public"."timestamps" 
FOR INSERT 
TO "authenticated" 
WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));

CREATE POLICY "Authenticated users can select their own video timestamps" ON "public"."timestamps" 
FOR SELECT 
TO "authenticated" 
...
CREATE POLICY "Enable delete for users based on user_id" ON "public"."timestamps" 
FOR DELETE 
USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));
...
CREATE POLICY "Users can SELECT their own user_playlists and public playlists" ON "public"."user_playlists"
FOR SELECT
USING (
...
CREATE POLICY "Users can INSERT user_playlists for playlists they created or are Public" ON "public"."user_playlists"
FOR INSERT
WITH CHECK (
    user_playlists.user_id = (select auth.uid())
    OR EXISTS (
...
CREATE POLICY "Users can DELETE their own user_playlists" ON "public"."user_playlists"
FOR DELETE
USING (
    user_playlists.user_id = (select auth.uid())
);
