-- Migration: Update timestamp functions to use auth.uid() for improved security
-- This removes user_id parameters and uses JWT authentication instead

-- Update insert_timestamp to use auth.uid()
DROP FUNCTION IF EXISTS public.insert_timestamp(uuid, text, numeric, timestamp with time zone, bigint, public.playlist_sorted_by, public.playlist_sort_order);
...
-- Update insert_timestamps to use auth.uid()
DROP FUNCTION IF EXISTS public.insert_timestamps(uuid, text[], numeric[], timestamp with time zone[]);
...
-- Update delete_timestamps to use auth.uid()
DROP FUNCTION IF EXISTS public.delete_timestamps(uuid, text[]);
...
GRANT EXECUTE ON FUNCTION public.insert_timestamp(text, numeric, timestamp with time zone, bigint, public.playlist_sorted_by, public.playlist_sort_order) TO authenticated;
GRANT EXECUTE ON FUNCTION public.insert_timestamps(text[], numeric[], timestamp with time zone[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_timestamps(text[]) TO authenticated;