...
-- cleanup_old_bucket_images() removed (Neon adaptation): queried
-- storage.objects / called storage.remove(), Supabase Storage internals
-- that don't exist on Neon. Old-image cleanup for the S3 bucket (Phase 3)
-- becomes app/Lambda-layer logic instead, called from wherever job
-- completion is handled in the new Lambda.
...
BEGIN
  ...
  -- Old-image bucket cleanup removed here (Neon adaptation) — was a call to
  -- cleanup_old_bucket_images(), which depended on Supabase's
  -- storage.objects/storage.remove(). S3 cleanup (Phase 3) happens in the
  -- app/Lambda layer instead.
...
-- pg_cron + net.http_post trigger removed (Neon adaptation): pg_net/vault
-- aren't supported on Neon anyway, and cron moves to AWS EventBridge +
-- Lambda (later phase) invoking start_image_processing_job() /
-- reset_stuck_image_processing_jobs() directly via a Postgres connection.
