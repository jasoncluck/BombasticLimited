...
-- ============================================================================
-- Create notification type enum (wrapped for idempotency, see 01)
DO $$ BEGIN
...
BEGIN
    RETURN EXISTS (
        ...
        WHERE id = (auth.user_id())::uuid AND account_type = 'admin'
    );
END;
$$;
...
SET
  search_path = '' AS $$
    ...
    WHERE un.user_id = (auth.user_id())::uuid 
    AND un.read = false 
...
BEGIN
    current_user_id := (auth.user_id())::uuid;
...
BEGIN
    ...
    INSERT INTO public.notifications (
        ...
        notification_type, notification_title, notification_message,
        notification_metadata, notification_action_url, notification_is_test,
        notification_start_datetime, notification_end_datetime, (auth.user_id())::uuid
    ) RETURNING id INTO new_notification_id;
    
    -- Bulk assign to all users
    INSERT INTO public.user_notifications (notification_id, user_id)
    SELECT new_notification_id, users.id
    FROM public.profiles users
    ON CONFLICT (notification_id, user_id) DO NOTHING;
...
-- =====================================================
-- STEP 9 (Neon adaptation): admin bootstrap and cron setup removed.
-- Admin bootstrap was a one-time UPDATE against Supabase's auth.users,
-- dead code on a fresh empty database — re-run manually (or via the
-- future Cognito Post-Confirmation Lambda's admin-email check) after the
-- real admin account signs up. setup_notification_cleanup_cron() called
-- pg_cron, not supported on Neon; the actual cleanup logic
-- (cleanup_expired_notifications_cron(), defined above) is unchanged and
-- gets invoked by a daily EventBridge-scheduled Lambda instead (later
...
VALUES
  (
    ...
    jsonb_build_object(
      'migration_name',
      '12_notifications_system_with_rpc_and_cron.sql',
      'completion_time',
      now(),
      'author',
      'jasoncluck',
      'description',
      'Complete notification system with optimized functions and indexes - FIXED index error'
    ),
    now()
  );
