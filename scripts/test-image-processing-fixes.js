...
const testFile = join(__dirname, '../supabase/tests/13_image_processing_job_completion.sql');
...
try {
  ...
  expectedPatterns.forEach((pattern, index) => {
    ...
    console.log(`${matches ? '✅' : '❌'} Test ${index + 1}: ${pattern.source}`);
    if (matches) passed++;
  });
...
migrations.forEach((migrationPath, index) => {
  try {
    ...
    const hasEntityValidation = content.includes('entity_exists') && content.includes('SELECT EXISTS');
    const hasSafeTypeCasting = content.includes('invalid_text_representation') && content.includes('EXCEPTION');
    const hasEntityUpdateFirst = content.indexOf('UPDATE') < content.indexOf('DELETE FROM');
    const hasBetterLogging = content.includes('RAISE LOG') || content.includes('RAISE WARNING');
    
    console.log(`Migration ${index + 1}: ${migrationPath.split('/').pop()}`);
    console.log(`  ${hasEntityValidation ? '✅' : '❌'} Entity existence validation`);
    ...
    if (hasEntityValidation && hasSafeTypeCasting && hasEntityUpdateFirst && hasBetterLogging) {
      migrationsPassed++;
    }
    console.log('');
...
console.log(`📊 Migration validation: ${migrationsPassed}/${migrations.length} files passed all checks`);
