...
async function testDurationCalculation() {
  ...
  try {
    ...
    const { data: testResults, error: testError } = await supabase.rpc('duration_to_seconds', {
    ...
    console.log(`   Expected: 5445 seconds → ${testResults === 5445 ? '✅ CORRECT' : '❌ INCORRECT'}\n`);
    ...
    if (playlists && playlists.length > 0) {
      playlists.forEach(playlist => {
        console.log(`   Playlist "${playlist.name}": ${playlist.duration_seconds} seconds`);
      });
    } else {
    ...
    console.log('   • Created automatic triggers to calculate duration when videos are added/removed');
    console.log('   • Updated get_playlist_data function to use pre-calculated duration');
    console.log('   • Fixed playlist thumbnails to prefer maxres URLs for better cropping quality');
